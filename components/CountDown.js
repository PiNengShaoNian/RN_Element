...
export default class CountDownView extends Component {
  ...
  getDateData(endTime) {
    ...
    timeLeft.min = parseInt(
      diff / 1000 / 60 - 24 * 60 * timeLeft.days - 60 * timeLeft.hours
    ) //减去天数和小时数的毫秒数剩下的毫秒，再除以每分钟的毫秒数，得到分钟数
    timeLeft.sec = parseInt(
      diff / 1000 -
        24 * 60 * 60 * timeLeft.days -
        60 * 60 * timeLeft.hours -
        60 * timeLeft.min
    ) //得到最后剩下的毫秒数除以1000 就是秒数，再剩下的毫秒自动忽略即可
    timeLeft.millisec = Math.floor(
      (diff -
        24 * 60 * 60 * 1000 * timeLeft.days -
        60 * 60 * 1000 * timeLeft.hours -
        60 * 1000 * timeLeft.min -
        timeLeft.sec * 1000) /
        10
    )
  ...
  render() {
    ...
    return (
      ...
      >
        {this.props.daysEnable ? (
          countDown.days > 0 ? (
            <Text style={this.props.daysStyle}>{countDown.days + days}</Text>
          ) : null
        ) : null}
